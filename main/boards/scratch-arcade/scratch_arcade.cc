...
#include "aec_audio_codec.h"

#define TAG "SratchArcade"

LV_FONT_DECLARE(font_puhui_16_4);
LV_FONT_DECLARE(font_awesome_16_4);

class SratchArcade : public WifiBoard {
  private:
    Button boot_button_;
    ...
    void InitializePowerManager() {
        power_manager_ = new PowerManager(BATTERY_LEVEL_PIN, BATTERY_EN_PIN);
    ...
    void InitializeButtons() {
        ...
        backlight_up_button_.OnClick([this]() {
            ...
            auto brightness = backlight->brightness() + 10;
            if (brightness > 100) {
                brightness = 100;
            }
            backlight->SetBrightness(brightness);
            GetDisplay()->ShowNotification(Lang::Strings::BRIGHTNESS + std::to_string(brightness));
        });

        backlight_up_button_.OnLongPress([this]() {
            power_save_timer_->WakeUp();
            GetBacklight()->SetBrightness(100);
            GetDisplay()->ShowNotification(Lang::Strings::MAX_BRIGHTNESS);
        });
        ...
        backlight_down_button_.OnLongPress([this]() {
            power_save_timer_->WakeUp();
            GetBacklight()->SetBrightness(1);
            GetDisplay()->ShowNotification(Lang::Strings::SLEEPING);
        });
    }
  ...
  public:
    SratchArcade()
        : boot_button_(BOOT_BUTTON_GPIO), volume_up_button_(VOLUME_UP_BUTTON_GPIO),
          volume_down_button_(VOLUME_DOWN_BUTTON_GPIO), backlight_up_button_(BACKLIGHT_UP_BUTTON_GPIO),
    ...
    virtual AudioCodec *GetAudioCodec() override {
        static AecAudioCodec audio_codec(AUDIO_INPUT_SAMPLE_RATE, AUDIO_OUTPUT_SAMPLE_RATE, AUDIO_I2S_SPK_GPIO_BCLK,
                                         AUDIO_I2S_SPK_GPIO_LRCK, AUDIO_I2S_SPK_GPIO_DOUT, AUDIO_I2S_MIC_GPIO_SCK,
                                         AUDIO_I2S_MIC_GPIO_DIN, AUDIO_INPUT_REFERENCE);
        return &audio_codec;
    }
