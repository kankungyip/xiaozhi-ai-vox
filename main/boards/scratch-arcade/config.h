#ifndef _BOARD_CONFIG_H_
#define _BOARD_CONFIG_H_

#include <driver/gpio.h>

#define USE_EXTERNAL_MIC (true)

#if CONFIG_USE_REALTIME_CHAT
#define AUDIO_INPUT_REFERENCE true
#else
#define AUDIO_INPUT_REFERENCE false
#endif
#define AUDIO_INPUT_SAMPLE_RATE 16000
#define AUDIO_OUTPUT_SAMPLE_RATE 24000

#if USE_EXTERNAL_MIC
#define AUDIO_I2S_MIC_GPIO_SCK GPIO_NUM_7
#define AUDIO_I2S_MIC_GPIO_DIN GPIO_NUM_2
#else
#define AUDIO_I2S_MIC_GPIO_SCK GPIO_NUM_47
#define AUDIO_I2S_MIC_GPIO_DIN GPIO_NUM_48
#endif

#define AUDIO_I2S_SPK_GPIO_DOUT GPIO_NUM_1
#define AUDIO_I2S_SPK_GPIO_BCLK GPIO_NUM_41
#define AUDIO_I2S_SPK_GPIO_LRCK GPIO_NUM_42

#define BOOT_BUTTON_GPIO GPIO_NUM_0
#define VOLUME_UP_BUTTON_GPIO GPIO_NUM_5
#define VOLUME_DOWN_BUTTON_GPIO GPIO_NUM_9
#define BACKLIGHT_UP_BUTTON_GPIO GPIO_NUM_4
#define BACKLIGHT_DOWN_BUTTON_GPIO GPIO_NUM_39

#define DISPLAY_BACKLIGHT_PIN GPIO_NUM_21
#define DISPLAY_MOSI_PIN GPIO_NUM_11
#define DISPLAY_CLK_PIN GPIO_NUM_12
#define DISPLAY_DC_PIN GPIO_NUM_45
#define DISPLAY_RST_PIN GPIO_NUM_46
#define DISPLAY_CS_PIN GPIO_NUM_10

#define DISPLAY_WIDTH 320
#define DISPLAY_HEIGHT 240
#define DISPLAY_MIRROR_X false
#define DISPLAY_MIRROR_Y true
#define DISPLAY_SWAP_XY true
#define DISPLAY_INVERT_COLOR true
#define DISPLAY_RGB_ORDER LCD_RGB_ELEMENT_ORDER_RGB
#define DISPLAY_OFFSET_X 0
#define DISPLAY_OFFSET_Y 0
#define DISPLAY_BACKLIGHT_OUTPUT_INVERT false
#define DISPLAY_SPI_MODE 0

#define POWER_SAVER_PIN GPIO_NUM_17
#define BATTERY_LEVEL_PIN GPIO_NUM_3
#define BATTERY_EN_PIN GPIO_NUM_18

#endif // _BOARD_CONFIG_H_
