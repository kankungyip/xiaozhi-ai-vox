...
#define TAG "NulllabAIVox4G"

LV_FONT_DECLARE(font_puhui_16_4);
LV_FONT_DECLARE(font_awesome_16_4);

class NulllabAIVox4G : public DualNetworkBoard {
  private:
    Button boot_button_;
    ...
    void InitializeButtons() {
        ...
        boot_button_.OnLongPress([this]() { // 长按且按下音量+键切换网络
            power_save_timer_->WakeUp();
            if (combine_buttons_) {
                combine_buttons_ = false;
                SwitchNetworkType();
            }
        });
        ...
        volume_up_button_.OnPressDown([this]() { combine_buttons_ = true; });
        volume_up_button_.OnPressUp([this]() { combine_buttons_ = false; });
  ...
  public:
    NulllabAIVox4G()
        : DualNetworkBoard(ML307_TX_PIN, ML307_RX_PIN, 4096), boot_button_(BOOT_BUTTON_GPIO),
